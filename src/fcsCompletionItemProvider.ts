...
const grammarNodes : GrammarType[] = [
    ...
    new GrammarType({ dot : 2, key : "Math.Interval1D", kind : GrammarKind.Module }),
    ...
    new GrammarType({ dot : 2, key : "Fcs.EngineeringQuantity.CrossSectionWarpInertia", description : "cross section inertia in warping [m⁶]"}),
    ...
    new GrammarType({ dot : 2, key : "Fcs.EngineeringQuantity.LoadForcePerLength", description : "load force per length intensity [kg/s²]"}),
    new GrammarType({ dot : 2, key : "Fcs.EngineeringQuantity.LoadMomentPerLength", description : "load moment per length intensity [kgm/s²]"}),
    ...
    new GrammarType({ dot : 2, key : "Fcs.EngineeringQuantity.MaterialElasticModulus", description : "material modulus of elasticity [kg/ms²]"}),
...
export class FcsCompletionItemProvider implements vscode.CompletionItemProvider {
    ...
    Promise<vscode.CompletionItem[]> {
        ...
        var priorWord: string = this.priorWord(document, position);
        var currentWord: string = this.currentWord(document, position);
        var dotBefore: boolean = this.dotBefore(document, position, currentWord);
        ...
        if (numberOfDot > 0) {
            filteredObjects = grammarNodes.filter( v => v.dot === numberOfDot && v.key.startsWith(priorWord));
        }
        ...
        if (!filteredObjects || filteredObjects.length === 0 ) {
            ...
            filteredObjects = grammarNodes.filter( v => v.dot === numberOfDot && v.key.startsWith( startWith ));
        }
    ...
    private dotBefore(doc: vscode.TextDocument, pos: vscode.Position, currentWord: string): boolean {
     ...
     */
    private priorWord(doc: vscode.TextDocument, pos: vscode.Position): string {
     ...
     */
    private currentWord(doc: vscode.TextDocument, pos: vscode.Position): string {
